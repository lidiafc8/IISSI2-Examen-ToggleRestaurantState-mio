...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  // solucion
  const patch = async (restaurant) => {
    try {
      await changeStatus(restaurant.id)
      await fetchRestaurants()
      showMessage({
        message: `Restaurant status successfully changed from ${restaurant.status} to ${restaurant.status === 'online' ? 'offline' : 'online'}`,
        type: 'success',
      ...
      console.log(error)
      showMessage({
        message: 'Restaurant status can not be changed',
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
